 ...
 */
import { DEFAULT_CONFIG, ANSI } from "./types.js";
...
function render(input) {
    ...
    if (config.showProject)
        line1Parts.push(renderProject(input, config));
    if (config.showSessionTime)
        line1Parts.push(renderSessionTime(input, config));
    if (config.costEnabled) {
        const cost = renderCost(input);
        if (cost)
            line1Parts.push(cost);
    }
...
function maybeAddNudge(output, input) {
