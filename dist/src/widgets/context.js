...
export function renderContextBar(ctx) {
    ...
    const color = pct >= config.contextCritAt
        ? ANSI.brightRed
...
export function renderContextFull(ctx) {
    ...
    const totalInputK = Math.round((usage.input_tokens + usage.cache_creation_input_tokens + usage.cache_read_input_tokens) / 1000);
