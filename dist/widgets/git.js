...
export function renderGit(cwd, config) {
    ...
    if (config.gitShowDirty) {
        const parts = [];
        if (git.staged > 0)
            parts.push(`${ANSI.brightGreen}+${git.staged}${ANSI.reset}`);
        if (git.modified > 0)
            parts.push(`${ANSI.brightYellow}~${git.modified}${ANSI.reset}`);
        if (git.untracked > 0)
            parts.push(`${ANSI.dim}?${git.untracked}${ANSI.reset}`);
        if (parts.length > 0)
            out += ` ${parts.join(" ")}`;
    }
    if (config.gitShowAheadBehind) {
        if (git.ahead > 0)
            out += ` ${ANSI.green}↑${git.ahead}${ANSI.reset}`;
        if (git.behind > 0)
            out += ` ${ANSI.red}↓${git.behind}${ANSI.reset}`;
    }
    return out;
}
