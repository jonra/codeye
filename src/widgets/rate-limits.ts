...
interface RateLimitBucket {
  utilization?: number;       // 0.0–1.0 (API format)
  used_percentage?: number;   // 0–100   (native stdin format)
  resets_at: string | number; // ISO 8601 string (API) or Unix epoch seconds (native)
}
...
function loadCache(): CachedRateLimits | null {
  try {
    if (!existsSync(CACHE_FILE)) return null;
    const cached = JSON.parse(readFileSync(CACHE_FILE, "utf8")) as CachedRateLimits;
...
function saveCache(data: RateLimitResponse): void {
  try {
...
async function fetchRateLimits(): Promise<RateLimitResponse | null> {
...
function colorForUtilization(pct: number): string {
  if (pct >= 80) return ANSI.brightRed;
  if (pct >= 60) return ANSI.brightYellow;
...
export async function renderRateLimits(input: StatusInput): Promise<WidgetOutput> {
  // Path B: native rate_limits field on stdin input
  const native = (input as any).rate_limits;
  if (native) {
    const parts: string[] = [];
    if (native.five_hour) parts.push(formatBucket("5h", native.five_hour));
    if (native.seven_day) parts.push(formatBucket("7d", native.seven_day));
  ...
  const parts: string[] = [];
  if (data.five_hour) parts.push(formatBucket("5h", data.five_hour));
  if (data.seven_day) parts.push(formatBucket("7d", data.seven_day));
  return parts.join(` ${ANSI.dim}·${ANSI.reset} `);
}
