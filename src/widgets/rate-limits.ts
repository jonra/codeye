...
export function renderRateLimits(input: StatusInput): WidgetOutput {
  const limits = (input as any).rate_limits;
  if (!limits) return "";

  const parts: string[] = [];
  if (limits.five_hour) parts.push(formatBucket("5h", limits.five_hour));
  if (limits.seven_day) parts.push(formatBucket("7d", limits.seven_day));
