...
interface StallState {
  ...
  lastChangeTs: number; // unix ms when tokens last changed
}

function totalTokens(input: StatusInput): number {
  return (input.context_window.total_input_tokens || 0) +
         (input.context_window.total_output_tokens || 0);
}

function loadStall(): StallState | null {
  try {
    if (existsSync(STALL_FILE)) {
      return JSON.parse(readFileSync(STALL_FILE, "utf8")) as StallState;
    }
  } catch { /* ignore */ }
  return null;
}

function saveStall(s: StallState): void {
  try {
...
export function renderStall(input: StatusInput): WidgetOutput {
