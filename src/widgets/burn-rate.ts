...
interface BurnSample {
  ts: number;        // unix ms
  tokens: number;    // total input tokens at this point
}
...
function loadState(): BurnState | null {
  try {
    if (!existsSync(STATE_FILE)) return null;
    return JSON.parse(readFileSync(STATE_FILE, "utf8")) as BurnState;
  } catch {
    return null;
  }
}

function saveState(state: BurnState): void {
  try {
...
export function renderBurnRate(input: StatusInput): WidgetOutput {
  ...
  if (dMin < 0.1 || dTokens <= 0) return ""; // Too short a window
