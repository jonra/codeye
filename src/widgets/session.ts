...
import { ANSI, StatusInput, HudConfig, WidgetOutput } from "../types.js";
...
function getSessionStart(sessionId: string): number {
  try {
    if (!existsSync(STATE_DIR)) {
      mkdirSync(STATE_DIR, { recursive: true });
    }
    if (existsSync(SESSION_FILE)) {
      const s = JSON.parse(readFileSync(SESSION_FILE, "utf8")) as SessionState;
      if (s.sessionId === sessionId) return s.startedAt;
    }
  } catch { /* ignore */ }
...
export function renderModel(input: StatusInput, _config: HudConfig): WidgetOutput {
  const name = input.model.display_name;
  return `${ANSI.brightWhite}${ANSI.bold}${name}${ANSI.reset}`;
}

export function renderProject(input: StatusInput, _config: HudConfig): WidgetOutput {
  const dir = input.workspace.current_dir || input.cwd;
  const project = basename(dir);
  return `${ANSI.dim}📁 ${project}${ANSI.reset}`;
}

export function renderSessionTime(input: StatusInput, _config: HudConfig): WidgetOutput {
